...
#+begin_src shell :exports code
  if [ -z "$INSIDE_EMACS" ] ; then
      ...
      # translate it to Windows console.  A shell buffer inside emacs has no
      # TTY, so no possibility to use winpty for unison.  There will be spurious
      # newlines in output...
      type -p winpty unison > /dev/null && alias unison='winpty unison'
  fi
#+end_src
...
#+begin_src shell :exports code
  if type -p emacsclient > /dev/null ; then
      ...
      # function would need to support CHPATH as well; I don't use CDPATH at the
